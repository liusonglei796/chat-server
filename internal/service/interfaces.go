...
import (
	...
	contactdto "kama_chat_server/internal/dto/respond/contact"
...
type UserService interface {
	// Login 密码登录
	Login(req auth.LoginRequest) (*userdto.LoginRespond, error)
	// SmsLogin 短信验证码登录
	SmsLogin(req auth.SmsLoginRequest) (*userdto.LoginRespond, error)
	// SendSmsCode 发送短信验证码
	SendSmsCode(telephone string) error
	// Register 用户注册
	Register(req auth.RegisterRequest) (*userdto.RegisterRespond, error)
	...
	GetUserInfo(requesterId, targetId string) (*userdto.GetUserInfoRespond, error)
	// GetPublicUserInfo 获取用户公开信息（查看他人）
	GetPublicUserInfo(targetId string) (*userdto.PublicUserInfoRespond, error)
}
...
type SessionService interface {
	...
	GetUserSessionList(ownerId string) ([]sessiondto.UserSessionListRespond, error)
	// GetGroupSessionList 获取用户群聊会话列表
	GetGroupSessionList(ownerId string) ([]sessiondto.GroupSessionListRespond, error)
	// DeleteSession 删除会话
...
type GroupService interface {
	...
	LoadMyGroup(ownerId string) ([]groupdto.MyGroupListRespond, error)
	// GetJoinedGroups 获取我加入的群组
	GetJoinedGroups(userId string) ([]groupdto.MyGroupListRespond, error)
	...
	GetPublicGroupInfo(groupId string) (*groupdto.PublicGroupInfoRespond, error)
	...
	GetGroupMemberList(userId, groupId string) ([]groupdto.GetGroupMemberListRespond, error)
...
// GetUserList 获取好友列表（分页）
	GetUserList(userId string, page, pageSize int) ([]userdto.MyUserListRespond, int64, error)
// GetGroupList 获取群组列表（分页）
	GetGroupList(userId string, page, pageSize int) ([]groupdto.MyGroupListRespond, int64, error)

	// GetFriendInfo 获取好友详情 (userId 必须与 friendId 是好友关系)
	GetFriendInfo(userId, friendId string) (contactdto.FriendInfoRespond, error)
	// GetGroupDetail 获取群聊详情 (userId 必须是群成员)
	GetGroupDetail(userId, groupId string) (contactdto.GroupDetailRespond, error)
	// DeleteContact 删除联系人
...
type ApplyService interface {
	...
	GetFriendApplyList(userId string) ([]applydto.FriendApplyListRespond, error)
	...
	GetGroupApplyList(userId, groupId string) ([]applydto.GroupApplyListRespond, error)
...
type MessageService interface {
	...
	GetMessageList(requesterId, partnerId string, page, pageSize int) ([]messagedto.GetMessageListRespond, error)
	// GetGroupMessageList 获取群聊消息记录 (userId 必须是群成员)
	GetGroupMessageList(userId, groupId string) ([]messagedto.GetMessageListRespond, error)
...
type UserAdminService interface {
	...
	GetUserListPaged(req adminreq.GetUserListRequest) (*admindto.PagedUserListRespond, error)
...
type GroupAdminService interface {
	...
	GetGroupInfoList(req adminreq.GetGroupInfoListRequest) (*admindto.GetGroupListWrapper, error)
	// DeleteGroups 批量删除群组
