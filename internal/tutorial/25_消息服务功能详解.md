...
| `RecallMessage` | 撤回已发送的消息。 | 用户长按消息点击“撤回”。 | 1. **查询消息**：根据 UUID 获取消息详情。<br>2. **权限校验**：确保发送者是本人。<br>3. **时间校验**：检查消息是否在撤回有效期内（如 2 分钟）。<br>4. **状态更新**：将消息状态更新为“已撤回”。<br>5. **消息推送**：通过 WebSocket 推送撤回通知给对方/群成员，前端替换展示为“XXX撤回了一条消息”。 |
