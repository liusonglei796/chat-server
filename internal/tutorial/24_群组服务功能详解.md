...
## 14. MuteMember
**函数签名**: `MuteMember(operatorId string, req request.MuteMemberRequest) error`

*   **核心目的**: 群成员禁言。
*   **应用场景**: 
    *   群主/管理员禁言违规成员。
*   **逻辑过程**:
    1.  **权限校验**: 确认操作者有权限（群主/管理员），且被操作者等级低于自己（不能禁言群主）。
    2.  **更新状态**: 更新 `GroupMember` 表的 `MuteUntil` 字段（禁言截止时间）。
    3.  **清理缓存**: 清理 `group_memberlist_<gid>` 缓存。
    4.  **消息推送**: (可选) 发送系统通知告知该成员已被禁言。

## 15. 核心实现要点总结 (Core Implementation Concepts)
