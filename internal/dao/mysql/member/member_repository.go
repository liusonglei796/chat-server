...
// groupUuid: 群组 UUID
// 返回: 群成员列表
func (r *groupMemberRepository) FindByGroupUuid(ctx context.Context, groupUuid string) ([]model.GroupMember, error) {
...
func (r *groupMemberRepository) FindMembersWithUserInfoPaged(ctx context.Context, groupUuid string, page, pageSize int) ([]model.GroupMemberWithUserInfo, int64, error) {
...
func (r *groupMemberRepository) FindByGroupAndUser(ctx context.Context, groupUuid, userUuid string) (*model.GroupMember, error) {
...
func (r *groupMemberRepository) CreateGroupMember(ctx context.Context, member *model.GroupMember) error {
...
// 用于解散群组时清理成员数据
func (r *groupMemberRepository) DeleteByGroupUuid(ctx context.Context, groupUuid string) error {
...
func (r *groupMemberRepository) DeleteByUserUuids(ctx context.Context, groupUuid string, userUuids []string) error {
...
// 用于批量删除群组时清理成员数据
func (r *groupMemberRepository) DeleteByGroupUuids(ctx context.Context, groupUuids []string) error {
	if len(groupUuids) == 0 {
		return nil
	}
...
// 用于批量操作时获取受影响的用户
func (r *groupMemberRepository) GetMemberIdsByGroupUuids(ctx context.Context, groupUuids []string) ([]string, error) {
	var members []string
	if len(groupUuids) == 0 {
		return members, nil
	}
...
func (r *groupMemberRepository) FindGroupUuidsByUserPaged(ctx context.Context, userUuid string, page, pageSize int) ([]string, int64, error) {
	var total int64
...
func (r *groupMemberRepository) UpdateMuteUntil(ctx context.Context, groupUuid, userUuid string, muteUntil *time.Time) error {
	if err := r.db.WithContext(ctx).Model(&model.GroupMember{}).
