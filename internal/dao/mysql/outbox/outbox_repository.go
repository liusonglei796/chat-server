...
// 本文件实现 OutboxRepository 接口，处理 outbox 表的数据库操作
package outbox
...
// outboxRepository OutboxRepository 接口的实现
...
// 确保实现 repository.OutboxRepository 接口
var _ repository.OutboxRepository = (*outboxRepository)(nil)

// Create 写入一条待发布事件
func (r *outboxRepository) Create(ctx context.Context, o *model.Outbox) error {
...
func (r *outboxRepository) MarkPublished(ctx context.Context, uuids []string) error {
	if len(uuids) == 0 {
		return nil
	}
...
func (r *outboxRepository) IncrementRetry(ctx context.Context, uuid string) error {
	err := r.db.WithContext(ctx).Model(&model.Outbox{}).
		Where("uuid = ?", uuid).
