# 16. 聊天服务器实现

> 本章以当前代码为准，解释聊天子系统如何通过 `ChatServer` 聚合并选择 `MessageBroker`，以及 Channel 模式下 `StandaloneServer` 如何管理在线连接与消息消费循环。

---

## 📌 学习目标

- 理解 `ChatServer` 的职责：按配置选择 Channel/Kafka Broker
- 理解 `MessageBroker` 接口对 WebSocket 网关的解耦作用
- 掌握 Channel 模式 `StandaloneServer` 的 Login/Logout/Transmit 并发模型
- 理解依赖注入：Repository/CacheService 如何传入 Broker

---

## 1. 代码位置（以当前实现为准）

- 服务器聚合与 Broker 选择：`internal/service/chat/server.go`
- Channel 模式 Broker：`internal/service/chat/channel_broker.go`
- Kafka 模式 Broker：`internal/service/chat/kafka_broker.go`

> 注意：本项目的 Channel Broker 文件名为 `channel_broker.go`，但文件头注释里仍可能出现 “channel_server.go” 字样；以真实路径为准。

---

## 2. MessageBroker：统一抽象

WebSocket 网关只依赖 `MessageBroker`，不关心底层是 Channel 还是 Kafka。

```go
// internal/service/chat/server.go

type MessageBroker interface {
    Publish(ctx context.Context, msg []byte) error
    RegisterClient(client *UserConn)
    UnregisterClient(client *UserConn)
    GetClient(userId string) *UserConn
    Start()
    Close()

    // 供 ws_gateway.go 在 Write() 中更新消息状态
    GetMessageRepo() repository.MessageRepository
}
```

---

## 3. ChatServer：聚合与模式选择

`ChatServer` 的职责是：

1. 接收配置与依赖（MessageRepo、GroupMemberRepo、CacheService）
2. 根据 `Mode` 选择创建 Channel 或 Kafka Broker
3. 提供统一的 `Start()/Close()` 生命周期

```go
// internal/service/chat/server.go

type ChatServer struct {
    Broker          MessageBroker
    KafkaClient     *KafkaClient
    messageRepo     repository.MessageRepository
    groupMemberRepo repository.GroupMemberRepository
    cacheService    myredis.AsyncCacheService
    mode            string
}

type ChatServerConfig struct {
    Mode            string
    MessageRepo     repository.MessageRepository
    GroupMemberRepo repository.GroupMemberRepository
    CacheService    myredis.AsyncCacheService
    KafkaHostPort   string
    KafkaTopic      string
}

func NewChatServer(cfg ChatServerConfig) *ChatServer {
    cs := &ChatServer{
        messageRepo:     cfg.MessageRepo,
        groupMemberRepo: cfg.GroupMemberRepo,
        cacheService:    cfg.CacheService,
        mode:            cfg.Mode,
    }

    if cfg.Mode == "kafka" {
        cs.KafkaClient = NewKafkaClient()
        cs.Broker = NewMsgConsumer(cs.KafkaClient, cs.messageRepo, cs.groupMemberRepo, cs.cacheService)
    } else {
        cs.Broker = NewStandaloneServer(cs.messageRepo, cs.groupMemberRepo, cs.cacheService)
    }

    return cs
}
```

---

## 4. Channel 模式：StandaloneServer 结构与依赖注入

Channel 模式 Broker 结构体为 `StandaloneServer`：

```go
// internal/service/chat/channel_broker.go

type StandaloneServer struct {
    Clients  sync.Map
    Transmit chan []byte
    Login    chan *UserConn
    Logout   chan *UserConn

    // 依赖注入（遵循依赖倒置）
    messageRepo     repository.MessageRepository
    groupMemberRepo repository.GroupMemberRepository
    cacheService    myredis.AsyncCacheService
}

func NewStandaloneServer(
    messageRepo repository.MessageRepository,
    groupMemberRepo repository.GroupMemberRepository,
    cacheService myredis.AsyncCacheService,
) *StandaloneServer {
    return &StandaloneServer{
        Transmit:        make(chan []byte, constants.CHANNEL_SIZE),
        Login:           make(chan *UserConn, constants.CHANNEL_SIZE),
        Logout:          make(chan *UserConn, constants.CHANNEL_SIZE),
        messageRepo:     messageRepo,
        groupMemberRepo: groupMemberRepo,
        cacheService:    cacheService,
    }
}
```

设计要点：

- 在线表使用 `sync.Map`，读写并发安全
- 业务侧不要直接依赖 Gorm/Redis 具体实现，而通过 Repository + CacheService 注入

---

## 5. Start：Login/Logout/Transmit 三路复用

`Start()` 是 Channel 模式的核心事件循环：

```go
// internal/service/chat/channel_broker.go

func (s *StandaloneServer) Start() {
    for {
        select {
        case client, ok := <-s.Login:
            if !ok {
                return
            }
            if client == nil {
                continue
            }
            s.Clients.Store(client.Uuid, client)
            _ = client.Conn.WriteMessage(websocket.TextMessage, []byte("欢迎来到kama聊天服务器"))

        case client, ok := <-s.Logout:
            if !ok {
                return
            }
            if client == nil {
                continue
            }
            s.Clients.Delete(client.Uuid)
            _ = client.Conn.WriteMessage(websocket.TextMessage, []byte("已退出登录"))

        case data, ok := <-s.Transmit:
            if !ok {
                return
            }

            var req request.ChatMessageRequest
            if err := json.Unmarshal(data, &req); err != nil {
                continue
            }

            switch req.Type {
            case message_type_enum.Text:
                s.handleTextMessage(req)
            case message_type_enum.File:
                s.handleFileMessage(req)
            case message_type_enum.AudioOrVideo:
                s.handleAVMessage(req)
            }
        }
    }
}
```

---

## 6. Broker 接口方法映射

Channel 模式实现了 `MessageBroker`：

```go
func (s *StandaloneServer) Publish(ctx context.Context, msg []byte) error {
    s.Transmit <- msg
    return nil
}

func (s *StandaloneServer) RegisterClient(client *UserConn) {
    s.Login <- client
}

func (s *StandaloneServer) UnregisterClient(client *UserConn) {
    s.Logout <- client
}

func (s *StandaloneServer) GetClient(userId string) *UserConn {
    value, ok := s.Clients.Load(userId)
    if !ok {
        return nil
    }
    return value.(*UserConn)
}

func (s *StandaloneServer) Close() {
    close(s.Login)
    close(s.Logout)
    close(s.Transmit)
}
```

---

## 7. 在 main.go 中启动

项目启动时创建 `ChatServer`，注入 Broker 到 `WsHandler`，并启动 `chatServer.Start()`：

```go
chatServer := chat.NewChatServer(chat.ChatServerConfig{
    Mode:            conf.KafkaConfig.MessageMode,
    MessageRepo:     repos.Message,
    GroupMemberRepo: repos.GroupMember,
    CacheService:    cacheService,
})
if conf.KafkaConfig.MessageMode == "kafka" {
    chatServer.InitKafka()
}
handlers := handler.NewHandlers(services, chatServer.GetBroker())
go chatServer.Start()
```

---

## ✅ 本节小结

- `ChatServer` 负责“选择并管理” Broker（Channel/Kafka）
- `StandaloneServer`（Channel 模式）用三通道模型串起登录/登出/消息消费
- Repo/CacheService 通过依赖注入进入 Broker，避免在业务里硬编码具体存储实现

下一章进入消息处理细节：单聊/群聊的入库、转发、以及 Redis 缓存更新策略。
