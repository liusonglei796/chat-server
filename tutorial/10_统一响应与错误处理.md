...
**核心设计**：
- `Wrap/Wrapf` - 像 `fmt.Errorf` 一样包装底层错误
- `Unwrap()` - 支持 `errors.Unwrap`/`errors.As` 追溯底层错误
- `GetCode()` - 从任意错误中提取业务码
- `IsNotFound()` - 检查是否为"未找到"错误（兼容 GORM）
...
> **架构变更说明**：响应处理从 `api/v1/` 移至 `internal/handler/`
...
> **路径变更**：从 `api/v1/validator.go` 改为 `internal/handler/validator.go`
...
> **路径变更**：从 `api/v1/user_info_controller.go` 改为 `internal/handler/user_handler.go`
...
继续学习 [11_用户模块API.md](11_用户模块API.md)，完善用户注册、登录等完整实现。
