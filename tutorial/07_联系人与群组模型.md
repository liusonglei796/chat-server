...
```mermaid
erDiagram
    UserInfo ||--o{ UserContact : has
    UserInfo ||--o{ ContactApply : sends
    UserInfo ||--o{ ContactApply : receives
...
CREATE TABLE `user_contact` (
  ...
  `contact_id` char(20) NOT NULL COMMENT '联系人ID',
...
| contact_id | 联系人 ID（用户或群组） |
| contact_type | 0=用户联系人，1=群组联系人 |
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='联系人申请表';
```
...
我们的 `internal/dao/mysql/gorm.go` 中的 `Init` 函数会自动处理迁移：
...
你已经完成了：
- [x] UserContact 联系人模型
- [x] ContactApply 好友申请模型（使用 applicant_id/target_id）
- [x] GroupInfo 群组模型
