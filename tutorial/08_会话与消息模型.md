...
> **重要变更**：字段名从 `user_id/target_id` 更改为 `send_id/receive_id`，并增加了冗余字段用于列表显示。
...
> **重要变更**：`Uuid` 使用雪花算法生成的 `bigint` 类型，而非字符串。
...
CREATE TABLE `message` (
  ...
  `av_data` varchar(1024) DEFAULT NULL COMMENT '通话传递数据',
  PRIMARY KEY (`id`),
...
`pkg/enum/message/message_type_enum/message_type.go`：
...
`pkg/enum/message/message_status_enum/message_status.go`：
...
```go
// internal/dto/request/av_data.go
type AVData struct {
    MessageId string `json:"messageId"` // PROXY 表示信令消息
    Type      string `json:"type"`      // start_call, receive_call, reject_call, end_call
...
我们已经在 `internal/dao/mysql/gorm.go` 中配置了所有模型的自动迁移：
