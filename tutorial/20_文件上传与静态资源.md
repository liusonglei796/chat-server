...
> 本教程将实现用户头像和聊天文件的上传功能，并配置静态资源服务。
...
**架构变化**：
- `Init` 函数现在接收 `*handler.Handlers` 参数并返回 `*gin.Engine`
- 不再使用全局变量 `GE`
- 通过路由管理器 `router.NewRouter(handlers)` 注入依赖
...
> **重要特性**：
> - 使用依赖注入模式，注入 Repository 和 CacheService
> - 返回新生成的文件名供前端使用
> - 使用 Magic Bytes 进行 MIME 类型校验
> - 生成唯一文件名防止覆盖和路径遍历攻击
...
**安全特性**：
- **Magic Bytes 校验**：读取文件前 512 字节检测真实 MIME 类型，防止扩展名欺骗
- **唯一文件名**：使用时间戳+随机字符串，防止覆盖和路径遍历
- **MIME 白名单**：头像仅允许 jpeg/png/gif 类型
- **回滚机制**：多文件上传失败时删除已上传的文件
...
你已经完成了：
- [x] 依赖注入模式的文件服务实现
- [x] Service 层文件保存（返回文件名）
- [x] Magic Bytes MIME 类型校验
- [x] 唯一文件名生成
- [x] 多文件上传回滚机制
- [x] 静态资源路由配置
...
继续学习 [21_HTTPS与安全配置.md](21_HTTPS与安全配置.md)，为你的服务加上安全锁。
