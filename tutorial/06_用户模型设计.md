# 06. 用户模型设计

> 本教程将设计用户信息模型，并使用 GORM 实现数据库迁移。

---

## 📌 学习目标

- 理解 GORM 模型定义规范
- 掌握数据库字段标签配置
- 理解自动迁移机制
- **掌握密码安全加密最佳实践**

---

## 1. 用户表设计

### 1.1 需求分析

用户表需要存储以下信息：

| 字段              | 类型           | 说明            |
| --------------- | ------------ | ------------- |
| uuid            | char(20)     | 用户唯一标识        |
| nickname        | varchar(20)  | 昵称            |
| telephone       | char(11)     | 手机号（登录凭证）     |
| email           | char(30)     | 邮箱            |
| avatar          | char(255)    | 头像 URL        |
| gender          | tinyint      | 性别（0-男，1-女）   |
| signature       | varchar(100) | 个性签名          |
| password        | varchar(100) | 密码（bcrypt 哈希） |
| birthday        | char(8)      | 生日（YYYYMMDD）  |
| last_online_at  | datetime     | 最后在线时间        |
| last_offline_at | datetime     | 最后离线时间        |
| is_admin        | tinyint      | 是否管理员         |
| status          | tinyint      | 状态（0-正常，1-禁用） |

> **安全提示**：`password` 字段使用 `varchar(100)` 存储 bcrypt 哈希值（固定 60 字符），而非明文密码。

---

## 2. 实现用户模型

### 2.1 internal/model/user_info.go

```go
package model

import (
    "database/sql"

    "golang.org/x/crypto/bcrypt"
    "gorm.io/gorm"
)

// UserInfo 用户信息模型
type UserInfo struct {
    gorm.Model                           // 内嵌 ID, CreatedAt, UpdatedAt, DeletedAt
    Uuid          string       `gorm:"column:uuid;uniqueIndex;type:char(20);comment:用户唯一id"`
    Nickname      string       `gorm:"column:nickname;type:varchar(20);not null;comment:昵称"`
    Telephone     string       `gorm:"column:telephone;index;not null;type:char(11);comment:电话"`
    Email         string       `gorm:"column:email;type:char(30);comment:邮箱"`
    Avatar        string       `gorm:"column:avatar;type:char(255);default:https://cube.elemecdn.com/0/88/03b0d39583f48206768a7534e55bcpng.png;not null;comment:头像"`
    Gender        int8         `gorm:"column:gender;comment:性别，0.男，1.女"`
    Signature     string       `gorm:"column:signature;type:varchar(100);comment:个性签名"`
    Password      string       `gorm:"column:password;type:varchar(100);not null;comment:密码"`
    Birthday      string       `gorm:"column:birthday;type:char(8);comment:生日"`
    LastOnlineAt  sql.NullTime `gorm:"column:last_online_at;type:datetime;comment:上次登录时间"`
    LastOfflineAt sql.NullTime `gorm:"column:last_offline_at;type:datetime;comment:最近离线时间"`
    IsAdmin       int8         `gorm:"column:is_admin;not null;comment:是否是管理员，0.不是，1.是"`
    Status        int8         `gorm:"column:status;index;not null;comment:状态，0.正常，1.禁用"`
    RawPassword   string       `gorm:"-" json:"-"` // 不存库，仅用于接收明文密码
}

// TableName 指定表名
func (UserInfo) TableName() string {
    return "user_info"
}

// BeforeSave 在创建和更新前自动加密密码
func (u *UserInfo) BeforeSave(tx *gorm.DB) (err error) {
    if u.RawPassword != "" {
        hash, err := bcrypt.GenerateFromPassword([]byte(u.RawPassword), bcrypt.DefaultCost)
        if err != nil {
            return err
        }
        u.Password = string(hash)
        u.RawPassword = "" // 清空明文
    }
    return nil
}

// CheckPassword 校验密码是否正确
func (u *UserInfo) CheckPassword(plaintext string) bool {
    err := bcrypt.CompareHashAndPassword([]byte(u.Password), []byte(plaintext))
    return err == nil
}
```

对应的建表 SQL（示意，实际以 AutoMigrate 结果为准）：

```sql
CREATE TABLE `user_info` (
  `id` bigint unsigned AUTO_INCREMENT,
  `created_at` datetime(3) NULL,
  `updated_at` datetime(3) NULL,
  `deleted_at` datetime(3) NULL,
  `uuid` char(20) DEFAULT NULL COMMENT '用户唯一id',
  `nickname` varchar(20) NOT NULL COMMENT '昵称',
  `telephone` char(11) NOT NULL COMMENT '电话',
  `email` char(30) DEFAULT NULL COMMENT '邮箱',
  `avatar` char(255) NOT NULL DEFAULT 'https://cube.elemecdn.com/0/88/03b0d39583f48206768a7534e55bcpng.png' COMMENT '头像',
  `gender` tinyint DEFAULT NULL COMMENT '性别，0.男，1.女',
  `signature` varchar(100) DEFAULT NULL COMMENT '个性签名',
  `password` varchar(100) NOT NULL COMMENT '密码',
  `birthday` char(8) DEFAULT NULL COMMENT '生日',
  `last_online_at` datetime DEFAULT NULL COMMENT '上次登录时间',
  `last_offline_at` datetime DEFAULT NULL COMMENT '最近离线时间',
  `is_admin` tinyint NOT NULL COMMENT '是否是管理员，0.不是，1.是',
  `status` tinyint NOT NULL COMMENT '状态，0.正常，1.禁用',
  PRIMARY KEY (`id`),
  INDEX `idx_user_info_deleted_at` (`deleted_at`),
  UNIQUE INDEX `idx_user_info_uuid` (`uuid`),
  INDEX `idx_user_info_telephone` (`telephone`),
  INDEX `idx_user_info_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户信息表';
```

---

## 3. 密码安全设计

### 3.1 为什么不能存储明文密码？

如果数据库泄露，所有用户密码将直接暴露。使用 bcrypt 单向哈希可以：
- 即使数据库泄露，攻击者也无法直接获取密码
- bcrypt 内置盐值（salt），防止彩虹表攻击
- 可配置的计算成本（cost），随硬件升级可增加难度

### 3.2 RawPassword 与 Password 分离

| 字段 | 用途 | 是否存库 |
|-----|------|---------| 
| `RawPassword` | 接收前端传来的明文密码 | ❌ `gorm:"-"` |
| `Password` | 存储 bcrypt 哈希值 | ✅ `varchar(100)` |

**工作流程**：
1. Service 层赋值 `user.RawPassword = req.Password`
2. GORM 调用 `BeforeSave` Hook
3. Hook 将 `RawPassword` 加密后赋值给 `Password`
4. 数据库存储的是哈希值

### 3.3 密码校验

```go
// 登录时校验密码
if !user.CheckPassword(req.Password) {
    return nil, errors.New("密码错误")
}
```

---

## 4. GORM 标签详解

### 4.1 常用标签

| 标签 | 说明 | 示例 |
|-----|------|------|
| `column` | 指定列名 | `gorm:"column:uuid"` |
| `type` | 指定数据类型 | `gorm:"type:varchar(20)"` |
| `not null` | 非空约束 | `gorm:"not null"` |
| `default` | 默认值 | `gorm:"default:0"` |
| `index` | 普通索引 | `gorm:"index"` |
| `uniqueIndex` | 唯一索引 | `gorm:"uniqueIndex"` |
| `comment` | 字段注释 | `gorm:"comment:用户ID"` |
| `-` | 忽略该字段 | `gorm:"-"` |

### 4.2 gorm.Model

内嵌 `gorm.Model` 会自动添加四个字段：
- `ID`：自增主键
- `CreatedAt`：创建时间
- `UpdatedAt`：更新时间
- `DeletedAt`：软删除时间


---

## 5. UUID 生成

### 5.1 pkg/util/random/random_int.go

> **路径变更**：从 `pkg/util/random/random.go` 改为 `pkg/util/random/random_int.go`

项目中已封装好的随机数生成工具：

```go
package random

import (
	"crypto/rand"
	"math/big"
	"time"
)

// GetRandomInt 生成指定位数的安全随机数字（用于验证码）
func GetRandomInt(length int) int {
	// 计算范围：例如 length=6 时，范围是 100000-999999
	min := int64(1)
	for i := 1; i < length; i++ {
		min *= 10
	}
	max := min * 10

	// 生成 [min, max) 范围的随机数
	rangeSize := big.NewInt(max - min)
	n, err := rand.Int(rand.Reader, rangeSize)
	if err != nil {
		return int(min) // fallback
	}
	return int(n.Int64() + min)
}

// GetNowAndLenRandomString 生成带时间戳前缀的随机字符串（用于 UUID）
// 格式: YYMMDD + 字母数字混合
// 示例: 241230AbCdE1234567
func GetNowAndLenRandomString(length int) string {
	result := make([]byte, length)
	const charset = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
	charsetLen := big.NewInt(int64(len(charset)))
	for i := range result {
		n, err := rand.Int(rand.Reader, charsetLen)
		if err != nil {
			result[i] = 'x'
			continue
		}
		result[i] = charset[n.Int64()]
	}
	return time.Now().Format("060102") + string(result)
}
```

**使用示例**：

```go
import "kama_chat_server/pkg/util/random"

// 生成用户 UUID（20位：U + 6位日期 + 13位随机）
uuid := "U" + random.GetNowAndLenRandomString(13)
// 示例输出: U241230AbCdE1234567

// 生成6位数字验证码
code := random.GetRandomInt(6)
// 示例输出: 123456
```

**UUID 命名规范**：

| 类型 | 前缀 | 格式 | 示例 |
|-----|-----|------|------|
| 用户 | U | U + 年月日 + 13位随机 | U241230AbCdE1234567 |
| 群组 | G | G + 年月日 + 13位随机 | G241230XyZ9876543 |
| 消息 | M | M + 年月日 + 11位随机 | M241230kLmN0123 |
| 会话 | S | S + 年月日 + 11位随机 | S241230pQrS4567 |

---

## 6. 数据库迁移

我们在 `internal/dao/mysql/init_mysql.go` 的 `Init` 函数中配置了自动迁移，并且 **不再使用全局 `GormDB/Repos`**：

> **当前仓库的真实形态**：`dao.Init()` 返回 `*mysql.Repositories`，由 `cmd/kama_chat_server/main.go` 作为组合根注入到 Service 层。

```go
package mysql

import (
    "kama_chat_server/internal/dao/mysql"
    "kama_chat_server/internal/model"
)

// Init 初始化数据库连接，自动迁移，并返回 Repositories
func Init() *mysql.Repositories {
    // ... 连接数据库 ...

    // 自动迁移
    err := db.AutoMigrate(
        &model.UserInfo{},
        &model.GroupInfo{},
        &model.Contact{},
        &model.Session{},
        &model.Apply{},
        &model.Message{},
        &model.GroupMember{},
    )
    if err != nil {
        // ... fatal log ...
    }

    return repository.NewRepositories(db)
}
```

只要将模型结构体指针传递给 `AutoMigrate`，GORM 就会在启动时自动同步数据库表结构。

---

## 7. 运行验证

```bash
cd cmd/kama_chat_server
go run main.go
```

查看 MySQL 数据库：
```sql
DESCRIBE user_info;
```

你将看到 `user_info` 表已经创建成功，包含所有定义好的字段。

---

## ✅ 本节完成

你已经完成了：
- [x] UserInfo 模型定义
- [x] GORM 标签配置
- [x] 了解 UUID 生成工具
- [x] 了解验证码生成工具
- [x] 验证数据库自动迁移

---

## 📚 下一步

继续学习 [07_联系人与群组模型.md](07_联系人与群组模型.md)，设计联系人、申请和群组模型。
